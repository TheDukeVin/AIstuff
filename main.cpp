...
const int numCarry = 5;

int numNodes[numlayers+1] = {numInputs + numCarry, 10, numOutputs + numCarry};
...
public:
    ...
    void backProp(double* inputs, double* expected){
        ...
        for(int n=seqLength-1; n>=0; n--){
            for(int i=0; i<numNodes[numlayers]; i++){
                nets[n].Dactivation[numlayers][i] = 2 * (nets[n].activation[numlayers][i] - expected[n*numOutputs + i]);
//                cout << "dir " << 2 * (nets[n].activation[numlayers][i] - expected[n*numOutputs + i]);
            }
            if(n == seqLength-1){
                for(int i=0; i<numCarry; i++){
